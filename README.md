...
| `/cron` | **Cron** *(write)* — scheduled jobs with per-job status; **create / edit / pause / resume / run-now / delete** jobs (applied via `hermes cron`). Job-builder form + inline edit (schedule, prompt, name, delivery), pre-filled from the job |
